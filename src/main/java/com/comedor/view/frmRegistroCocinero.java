/**
 *
 * @author Diego Cespedes
 */


package com.comedor.view;

import com.comedor.controlador.DAO.CocineroDao;
 ...
 */
@Theme("mytheme")
public class frmRegistroCocinero extends Composite implements View {
    ...
    private static CocineroDao cocineroBD = null;
    ...
    protected void init(VaadinRequest request) {
        ...
        Titulo.setValue("Registrar Cocinero");
        ...
        button.addClickListener(e -> {
            if (validarDatos(cedula.getValue(), name.getValue(), surname.getValue(), street.getValue(), phone.getValue(), email.getValue(), groupGenero.getValue(), cboEstadoCivil.getValue().getPosicion())) {
                try {
                    ...
                    if (ci > 0) {
                        cocinero = new Cocinero(ci, name.getValue(), surname.getValue(), street.getValue(), phone.getValue(), email.getValue(), groupGenero.getValue(), cboEstadoCivil.getValue());
                        layout.addComponent(new Label("Cliente grabado en la Base de Datos"));
                        try {
                            if (conexion != null) {
                                int result = cocineroBD.insertarCocinero(cocinero);
                                if (result > 0) {
                                    Notification notification = new Notification("Registro guardado exitosamente", Notification.Type.ASSISTIVE_NOTIFICATION);
                                    notification.show(cocinero.toString(), Notification.Type.ASSISTIVE_NOTIFICATION);
